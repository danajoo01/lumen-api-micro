    ...
    public function allcategory(Request $req)
    {
        ...
        $this->validate($req, [
         ...
         if ($req->has("sort_by") && $req->has("sort_mode")){
             $paginator = Category::orderBy($req->sort_by,$req->sort_mode);
         }else{
             $paginator = Category::orderBy("id","asc");
         }
         if ($req->has("category_id")){
            $paginator->where(["category_id"=>intval($req->category_id)]);
        }
         ...
         $data = array_filter($paginator->items(),function ($v){
            $v->thumbs = env("ASSET_URL").('images/all/'. $v->thumbs);
             return $v;
         });
